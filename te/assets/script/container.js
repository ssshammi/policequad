...
//mission1();
$j("#gloss_btn").qtip({    content: {text: 'పదకోశం'}	});
//$j("#audio_btn").qtip({    content: {text: 'Obtuse Angle Marker'}	});
$j("#help_btn").qtip({    content: {text: 'ఆడటం  ఎలాగ'}	});
$j("#menu_btn").qtip({    content: {text: 'మెను'}	});
...
//	currentCulpret ="fig001";
	...
	$("#feedback").hide();
	$("#trans").hide();
	$j(".mission").html("మిషన్ 2");
	$j(".level").html("లెవెల్  "+currentLevelDisplay2[CurrentStage-1]);
...
$j(CurrentMission2+".ToolMain2:visible").freetrans('destroy');
	...
	$j(CurrentMission2 +'.ask').unbind().bind('click',askClick);
	$j(CurrentMission2+".tool2:eq(0)").qtip({    content: {text: 'ఒబ్ట్యూస్ (గురు) కోణం(లు)'}	});
	$j(CurrentMission2+".tool2:eq(1)").qtip({    content: {text: 'రైట్ యాంగిల్(స్) (లంబ కోణం(లు))'}	});
	$j(CurrentMission2+".tool2:eq(2)").qtip({    content: {text: 'యాక్యూట్ (తీవ్ర) కోణం(లు)'}	});
	$j(CurrentMission2+".tool2:eq(3)").qtip({    content: {text: 'రిఫ్లెక్స్ (ప్రతిబింబ) కోణం(లు)'}	});
	
	/* 	$j(CurrentMission2+'.ask').on('click', function() {
		
	$j(CurrentMission2+'.clue').hide();
	if(!isReleased){
	alertify.set({ labels: {ok: "అవును", cancel : "రద్ుద చేయండి"}  });
...
function askClick() {
	...
	var responseColor='';
	var QuestionText = 'అపరాధి ద్గగర ఉందా ' +statment.toLowerCase();
	...
	if(ResponseText=="నాకు అరథం కాలేద్ు"){
	...
	$j(CurrentMission2+'.grid:visible').not('.released').each(function (){
		...
		if(allselected==$j(CurrentMission2+'.grid:visible').not('.released').length){
			alertify.alert('అనుమానితులందరికీ ఈ లక్షణం ఉన్నట్లుంది.  ఇంకో ప్రశ్న అడగండి.',function(){
		...
		if(noneselected==$j(CurrentMission2+'.grid:visible').not('.released').length){
			alertify.alert('ఈ లక్షణంతో ఎలాంటి అనుమానితుడు ఉన్నట్లుంది. మరొక ప్రశ్న అడగండి',function(){
				/*code added by abhishek shrimali to bind unbind release and askbutton*/
				$j(CurrentMission2+'.ask').unbind().bind('click',askClick);
				$j(CurrentMission2+'.ask').css('opacity','1');
				$j(CurrentMission2+'.release').unbind();
...
function enableTools(){
	...
	$j(CurrentMission2+'.bucket:eq(0)').append('<select id="bucket1"><option value=""></option><option value="atleast">కనీసం</option><option value="lessthan">కంటే తక్కువ</option><option value="morethan">మించి</option><option value="exactly">ఖచ్చితంగా</option><option value="all">అన్ని</option><option value="no">సున్నా</option> </select>');
	...
	$j(CurrentMission2+'.bucket:eq(2)').append('<select id="bucket3"><option value=""></option><option value="equal">సమాన</option><option value="parallel">సమాంతర</option><option value="right">లంబ</option><option value="reflex">ప్రతిబింబ</option><option value="obtuse">గురు</option><option value="acute">అలఎ</option></select>');
	...
	$j(CurrentMission2+'.bucket:eq(3)').append('<select id="bucket4"><option value=""></option><option value="sides">భుజాలు</option><option value="angles">కోణాలు</option><option value="pairsofsides">భుజాల జతలు</option></select>');
...
PlaySound(valueObj);
if (valueObj =="Y"){countYes++; return "అవును";}
if (valueObj =="N"){countN0++; return "కాదు";}
if (valueObj =="Invalid" || valueObj == undefined){countNA++; 
	...
	calculateScore2();
	return "నాకు అర్థం కాలేదు";
}
if (valueObj =="NA"){ countTBD++; return "నాకు అర్థం కాలేదు";}
if (valueObj =="DK"){ countTBD++; return "నాకు అర్థం కాలేదు";}
if (valueObj =="DA"){ countIDK++; return "నాకు అర్థం కాలేదు";}
}
...
function PlaySound(respo){
	...
	function releaseClick() { 	
			...
			if( $j(CurrentMission2+'.grid.selected').length >= CurrentSet.length){
				alertify.alert('అనుమానితులందరినీ విడుదల చేయడం సాధ్యం కాదు వాళ్ళల్లో ఒకరు ఒక అపరాధి.');
				return;
			 }
			if( $j(CurrentMission2+'.grid.selected').length ==0){
				alertify.alert('దయచేసి విడుదల చేసేందుకు ఒక అమాయక అనుమానితుడు (లు) ని ఎంచుకోండి.');
				return;
			 }
			{ // $j(CurrentMission2+'.grid.selected').hide();
			  ...
			  if($j(CurrentMission2+'.grid.selected').length == CurrentSet.length-1){
				...
				var res= (culpret== currentCulpret )?"మీరు అపరాధిని పట్టుకున్నారు.":", అయ్యో ... మీరు అపరాధిని కూడా విడుదల చేసినట్లనిపిస్తోంది! వచ్చే సారి కేసుతో బెటర్ లక్!";
	...
	if(currentScore==coinsValue[CurrentStage-1]){
		$j(".mtch_txt").html("ఏదీ మ్యాచ్ కాలేదు "); $j(".job").html("అయ్యయ!");$j(".solved").html("దయచేసి మళ్ళీ ప్రయత్నించండి!");
		...
		alertify.confirm("మీ కేసు పరిష్కరించబడలేదు, మీరు ముగించాలని ఖచ్చితంగా అనుకుంటున్నారా?", function(e){ 
			if (e) {
				resetLevel();
				mission2();
			}else{
		...
		alertify.alert("తొందరపడవద్దు – ఈసారి మరిన్ని ప్రశ్నలు అడగండి!");
		return true;
	}	
	
	if(currentScore>coinsValue[CurrentStage-1]){
		if(!isMatched){
			//$j(".mtch_txt").html("ఏదీ మ్యాచ్ కాలేదు "); $j(".job").html("చక్కటి ప్రయత్నం! ");$j(".solved").html("దయచేసి మళ్ళీ ప్రయత్నించండి!");
			alertify.alert("అయ్యో! మీరు అపరాధిని కనుక్కోలేకపోయారు! వచ్చే సారి కేసుతో బెటర్ లక్!",function(){
				$j(".mtch_txt").html("ఏదీ మ్యాచ్ కాలేదు "); $j(".job").html("అయ్యయ!");$j(".solved").html("మీరు అపరాధిని పట్టుకున్నారు.");
				getFeedbackAction();assessment_negative.play();
			});
		}else{ assessment_positive.play();
				alertify.alert('అద్భుతమైన పని! మీరు కేసు పరిష్కరించారు! ',function(){
   ...
   $j(".lvlScreen .level2").html("లెవెల్  "+ (currentLevelDisplay2[CurrentStage-1]+1));
...
var Correct = ($j(this).attr('resp') ==(finalQuestionsXML.find('mergedBucket:contains('+$j(this).attr('statment')+')').parent().find(suspected).text().replace("Y","అవును").replace("N","కాదు")));
if(!Correct) $j(this).find('.spot').css("background-color","rgb(248, 13, 13)");
	...
	function callbackPuff(){ 
		...
		if(currentScore <= coinsValue[CurrentStage-1]){
			isMatched=false;
			alertify.alert('అయ్యయ! మీరు మీ పరశ్ాలనిాంట్ినీ పూరిత చేస సుకునాారు. వచేే సారి కేసుతో బెట్ర్ లక్!',getFeedback);
...
var Hyperlinks= [["ప్యారెలల్ పెయిర్స్ (సమాంతర జతల) భుజాలు",'<a class="example-image-link" target="_blank" href="images/G_parallelSide.png">ప్యారెలల్ పెయిర్స్ (సమాంతర జతల) భుజాలు</a>'],
["స్ట్రెయిట్ సైడ్(స్) (నిటారు భుజం (లు))",'<a class="example-image-link" target="_blank"  href="images/G_straightSide.png">స్ట్రెయిట్ సైడ్(స్) (నిటారు భుజం (లు))</a>'],
["కర్వడ్ సైడ్(స్) (ఒంపుతిరిగిన భుజం (లు))",'<a class="example-image-link" target="_blank"  href="images/G_curvedSide.png">కర్వడ్ సైడ్(స్) (ఒంపుతిరిగిన భుజం (లు))</a>'],
["సమాన భుజం(ల) జత",'<a class="example-image-link" target="_blank"  href="images/G_equalSide.png">సమాన భుజం(ల) జత</a> '],
["యాక్యూట్ (తీవ్ర) కోణం(లు)",'<a class="example-image-link" target="_blank"  href="images/G_acuteAngle.png">యాక్యూట్ (తీవ్ర) కోణం(లు)</a>'],
["ఒబ్ట్యూస్ (గురు) కోణం(లు)",'<a class="example-image-link" target="_blank"  href="images/G_obtuseAngle.png">ఒబ్ట్యూస్ (గురు) కోణం(లు)</a>'],
["రైట్ యాంగిల్(స్) (లంబ కోణం(లు))",'<a class="example-image-link" target="_blank"  href="images/G_rightAngle.png">రైట్ యాంగిల్(స్) (లంబ కోణం(లు))</a>'],
["రిఫ్లెక్స్ (ప్రతిబింబ) కోణం(లు)",'<a class="example-image-link" target="_blank"  href="images/G_reflexAngle.png">రిఫ్లెక్స్ (ప్రతిబింబ) కోణం(లు)</a>'],
["యాక్యూట్ (తీవ్ర) కోణం(లు)",'<a class="example-image-link" target="_blank"  href="images/G_acuteAngle.png">యాక్యూట్ (తీవ్ర) కోణం(లు)</a>'],
["ఒబ్ట్యూస్ (గురు) కోణం(లు)",'<a class="example-image-link" target="_blank"  href="images/G_obtuseAngle.png">ఒబ్ట్యూస్ (గురు) కోణం(లు)</a>'],
["రైట్ యాంగిల్(స్) (లంబ కోణం(లు))",'<a class="example-image-link" target="_blank"  href="images/G_rightAngle.png">రైట్ యాంగిల్(స్) (లంబ కోణం(లు))</a>'],
["రిఫ్లెక్స్ (ప్రతిబింబ) కోణం(లు)",'<a class="example-image-link" target="_blank"  href="images/G_reflexAngle.png">రిఫ్లెక్స్ (ప్రతిబింబ) కోణం(లు)</a>']
