...
$j(CurrentMission1+".ToolMain2:visible").freetrans('destroy');
	...
	alertify.set({ labels: {ok: "ठीक", cancel : "वापसी"}  });
...
function GeneratingShapes(){ setShapesM1 = []; var tempClueOrder=[];
	switch(currentstage){
		case 1:{
			//M1Stage1
			//step1- Give me a random shape (fig?) with property  value Yes from main clue Randomly
			...
			var IdealShapes =[10,1,3,2]; // generateshapes like this
			 var tempArrayForSor=[]
			...
			//for (var i=0;i<TotalShapes-1;i++){
				// XMLNode = finalCluesXML.find('mergedBucket:contains('+tempArrayForSor[i]+')').parent();
			//}
			/* for (var i=0;i<TotalShapes-1;i++){
