...
if (valueObj =="DA"){ countIDK++; return "I don't understand";}
	...
	function releaseClick() { 	
			...
			if( $j(CurrentMission2+'.grid.selected').length >= CurrentSet.length){
				alertify.alert('Not all suspects can be released one is a culpret, Please Identify him.');
				return;
			 }
			if( $j(CurrentMission2+'.grid.selected').length ==0){
				alertify.alert('ద్యచేసి విడుద్ల చేస ంద్ుకు ఒక అమాయక అనుమానిత్ుడు (లు) ని ఎంచుక ండి.');
				return;
			 }
	...
	if(currentScore==coinsValue[CurrentStage-1]){
		$j(".mtch_txt").html("ఏదీ మ్యాచ్ కాలేదు "); $j(".job").html("అయ్యయ!");$j(".solved").html("You have exhausted all appempts!");
		getFeedbackAction();
	}
	...
	if(currentScore>coinsValue[CurrentStage-1]){
		if(!isMatched){
			...
			alertify.alert('అయ్యయ! "మీరు అపరాధిని పట్టుకున్నారు.".Better luck with the next case!',function(){
...
["reflex angles",'<a class="example-image-link" target="_blank"  href="images/G_reflexAngle.png">acute angles</a> ']]; 