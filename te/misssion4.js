...
return [computerShape,computerSelectedPoints,aSelectedVertex,compVFound,playerVFound];
  ...
  $j("#gloss_btn,#help_btn,#menu_btn").show();
	$j(".teleporter1Shape").on('DOMNodeInserted', function(e) {
    $j(e.target).find('.example-image-link').each(function() { //console.log("hi");
       $j(this).featherlight();;
    });
	});
  jQuery(".feed:eq(3) .cont_btn").click(function() {    
  ...
  var ShapeBox =["చతురస్రం","చతురస్రం","దీర్ఘచతురస్రం","దీర్ఘచతురస్రం","సమచతుర్భుజం (రోంబస్)","సమాంతర చతుర్భుజం (పారెలల్లోగ్రాం)"];
  ...
  // console.log("changed");
   $j(".mission").html("మిషన్ 4");
	$j(".level").html("లెవెల్  "+currentLevelDisplay4[level]);
	currentScore =MaxScore4[level];
	$j(".scoretxt").html(currentScore);
		$j(".star").html(" "+ gameScore);
	//$j(".scoretxt2").html(currentScore);
    ...
    if(level === 0) {
      ...
      gridSize = 5;
		$j(".teleporter1Shape").html("మీ టెలిపోర్టర్ ఒక  <a class='example-image-link' target='_blank' href='images/G_square.png'><b>చతురస్రం</b></a>");
		$j(".oppText").html("ఒక శత్రువు టెలిపోర్టర్ ఉంది <a class='example-image-link' target='_blank' href='images/G_square.png'><b>చతురస్రం</b></a>");
     ...
     // teleporter1Shape.style.display = "block";
      ...
      gridSize = 5;
     $j(".teleporter1Shape").html("మీ టెలిపోర్టర్ ఒక  <a class='example-image-link' target='_blank' href='images/G_rectangle.png'><b>దీర్ఘచతురస్రం</b></a>");
    $j(".oppText").html("ఒక శత్రువు టెలిపోర్టర్ ఉంది <a class='example-image-link' target='_blank' href='images/G_rectangle.png'><b>దీర్ఘచతురస్రం</b></a>");
     // teleporter1Shape.style.display = "block";
    } else if(level === 3) {
      legalShapes = squares5.concat(rectangles5);
     ...
     // teleporter1Shape.style.display = "block";
      ...
      computerShape = temp[Math.floor(Math.random()*temp.length)];
      gridSize = 5;
	    $j(".teleporter1Shape").html("మీ టెలిపోర్టర్ ఒక  <a class='example-image-link' target='_blank' href='images/G_rhombus.png'><b>సమచతుర్భుజం (రోంబస్)</b></a>");
		$j(".oppText").html("ఒక శత్రువు టెలిపోర్టర్ ఉంది <a class='example-image-link' target='_blank' href='images/G_rhombus.png'><b>సమచతుర్భుజం (రోంబస్)</b></a>");
     ...
     // teleporter1Shape.style.display = "block";
      ...
      var temp = parallelograms4;//squares4;
      var r = Math.random();
      if(r > 0.25 && r <= 0.5) {
        temp = rectangles4;
	   ...
	   temp = parallelograms4;
      } else if(r > 0.75) {
        temp = rhombuses4;
      }
      computerShape = temp[Math.floor(Math.random()*temp.length)];
      gridSize = 4;
	  	    $j(".teleporter1Shape").html("మీ టెలిపోర్టర్ ఒక  <a class='example-image-link' target='_blank' href='images/G_parallelogram.png'><b>సమాంతర చతుర్భుజం (పారెలల్లోగ్రాం)</b></a>");
			$j(".oppText").html("ఒక శత్రువు టెలిపోర్టర్ ఉంది <a class='example-image-link' target='_blank' href='images/G_parallelogram.png'><b>సమాంతర చతుర్భుజం (పారెలల్లోగ్రాం)</b></a>");
...
//------------------------------------------------------------------------
	...
	$j("#mission4 .opp .grid"+gridSize+" .cell").each(function (index){
			$(this).unbind();
		$(this).mouseenter(toggle).mouseleave(toggleback);
		$(this).one('click',evaluatePlayer);
		$(this).children('div:gt(0)').hide();
		if(index%gridSize ==0) {i++; j=0;}
		this.gridx = j;
		this.gridy = gridSize-i;
...
/*     p1.style.color="white"; */
  ...
  function selectedPoints(e) { 
	...
	else if(computerSelectedPoints.length>=4){
		alertify.alert("ఆకారం యొక్క నాలుగు వర్టిసెస్ ని మాత్రమే ఎంచుకోండి.");
	  }
    else {
      e.currentTarget.children[0].style.display = "none";
      e.currentTarget.children[1].style.display = "none";
  ...
  function draw() {
    ...
    if(!valid) {
      alertify.alert("అయ్యో!  ఈ ఆకారం అయి ఉండాలి "+ShapeBox[currentLevel]+". మళ్ళీ ప్రయత్నించండి!");
    } else {
  ...
  //alert("dsfsd1");
	...
	if(playerVFound>=2){		
			...
			if(allowedFlag==0){
				...
				if(allowedFlag==0){
					...
					if(currentScore >coinsValue4[currentLevel]){
					if(isBonus==1){
						$j(".mission4 .coin:eq(6)").effect( "explode", {}, 500, callbackPuff4 );
						isBonus=0;
					}else{
					    $j(".mission4 .coin:eq("+--stageCoins+")").effect( "explode", {}, 500, callbackPuff4 ); 	
					}
					}
    ...
    if(!exists) {
     ...
     // thinkingBox.style.transition = "display 0.5s ease-in-out";
      setTimeout(compPick, 1000);
	  	setTimeout(function(){$j(".Thinking").show();},500);
    }
...
$j("#feedback").hide();
	 ...
	 legalShapes =[];
	 //rethink()
 	 if(currentLevel == 6){currentLevel =5;   $j(".nextcase").hide();
	 $j(".play").hide();//alertify.alert("Mission Completed!");
	 levelChange(currentLevel);
	 }else{
		levelChange(currentLevel);
	} 
...
function getFeedback4(){
	...
	if(MatchedH!="player"){ assessment_negative.play();$j(".mtch_txt").html("రా-హాకా సైన్యం గెలిచింది!"); $j(".job").html("చక్కటి ప్రయత్నం! ");$j(".solved").html("దయచేసి మళ్ళీ ప్రయత్నించండి!");
	...
	//$j( ".additionalTextVal").hide();
	 ...
	 $j(".mtch_txt").html("మీరు గెలిచారు"); $j(".job").html("అమోఘం!");$j(".solved").html("మీరు శతృవు ఫ్రేమ్ ని డియాక్టివేట్ చేసారు!");
	 $j( ".additionalTextVal").html("మీరు పరిష్కరించడానికి ఇంకొక కేసు ఉన్నట్లుంది. ");
	 if(currentLevel == 5){
	 $j( ".additionalTextVal").html("<br>	&ensp;	&ensp;	&ensp;	&ensp;	&ensp;మిషన్ ముగింపు!");
	...
	if(isBonus){
		...
		loadedComps["feedback"].getStage().getSymbol("m04_feedback").getSymbol("anim_star").play(1);
		}, 1000);
		setTimeout(function () {
		loadedComps["feedback"].getStage().getSymbol("m04_feedback").$("mc_PerfectScore").show();
		loadedComps["feedback"].getStage().getSymbol("m04_feedback").getSymbol("mc_PerfectScore").play(1);
		}, 1500);
	}
	$j(".mission4 .star:eq(0)").html(" "+ gameScore);
	$j(".feed:eq(3) "+".scoretxt").html(currentScore);
	$j(".feed:eq(3) "+".scoretxt").effect( "highlight",200);
	$j(".lvlScreen .level2").html("లెవెల్  "+ (currentLevelDisplay4[currentLevel]+1));
	//alert("అలాగే");
 ...
 // alertify.alert(st +" wins!",getFeedback4);
  ...
  var plot = 'M '+vertices[0][0]*multiplier+' '+vertices[0][1]*multiplier+' L'+vertices[1][0]*multiplier+' '+vertices[1][1]*multiplier+' L'+vertices[2][0]*multiplier+' '+vertices[2][1]*multiplier+' L'+vertices[3][0]*multiplier+' '+vertices[3][1]*multiplier+' ';
	...
	if(st!="player")$j(".nextcase .txt").html("Play Again"); 
		//getFeedback4();
	alertify.alert((st=='player'?"మీరు గెలిచారు!":"రా-హాకా సైన్యం గెలిచింది!") ,getFeedback4);	
	//	MatchedH +" wins!"
