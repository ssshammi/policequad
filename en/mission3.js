...
function enableTools3(){
	...
	//rightAndge tool
	$j(CurrentMission+'.bucket:eq(0)').append('<select id="bucket1"><option value=""> </option><option value="atleast">At Least</option><option value="lessthan">Less Than</option><option value="morethan">More Than</option><option value="exactly">Exactly</option><option value="all">All</option><option value="no">No</option> </select>');
	...
	*/
	...
	$j(CurrentMission+'.bucket:eq(3)').append('<select id="bucket4"><option value="">           </option><option value="sides">side(s)</option><option value="angles">angle(s)</option><option value="pairsofsides">pairs of sides</option></select>');
...
function callbackPuff3(){
	...
	if(currentScore <= coinsValue3[currentStage-1]){
			isMatched=false;
			shapeOrigin='Block';
			alertify.alert('Oops! You have exhausted all tokens. Better luck with the next case!',showFeedback2);			
...
$j( ".genrate" ).delay(1000).effect( "transfer",{ to: ".allowed", className: "ui-effects-transfer_allow" }, 1000).delay(800).effect( "transfer",{ to: ".allowed", className: "ui-effects-transfer_allow" }, 500);
	...
	$j(".lvlScreen .level2").html("LEVEL "+ (currentLevelDisplay3[currentStage-1]+1));
 ...
 */	
	$j(".mtch_txt").html("Allowed!"); $j(".job").html("Great Job!");$j(".solved").html("You managed to send your secret agent in!");
...
//$j( ".genrate" ).addClass("ui-effects-transfer_allow");
	...
	$j(".feed:eq(2) "+".scoretxt").effect( "highlight",200);
//setTimeout(function(){
	$j(".mtch_txt").html("Blocked"); $j(".job").html("Oops!");$j(".solved").html("You could not find a suitable Judic agent!");
	 $j( ".additionalTextVal").show();
