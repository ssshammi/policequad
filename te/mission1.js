...
//	console.log("Final Found "+foundNow+" and this is  the value is "+allowedClueCount);
	...
	var QuestionText = 'అపరాధి ద్గగర ఉంది ' + appendStatement;
 ...
 $j(CurrentMission1+'.release').click(function() { 	 $j(CurrentMission1 +'.ask').show();  
	 ...
	 //$j(CurrentMission1+'.release').trigger("click");
		...
		alertify.confirm("మీ కేసు పరిష్ోరించబడలేద్ు, మీరు ముగించాలని ఖచ్చేత్ంగా అనుకుంట్ునాారా?", function(e){ 
			if (e) {
				//resetLevel1();
				var res= "అయ్యయ! మీరు అపరాధిని కనుక ోలేకను్యారు! వచేే సారి కేసుతో బెట్ర్ లక్!";
				isMatched = false;
				alertify.set({ labels: {ok: "కొనసాగించు",cancel : "అవును"}  });//,onok:
		...
		if($j(CurrentMission1+'.grid.selected.released').length == TotalShapes-1){
				...
				var res= (culpret== currentCulpret1 )?"అద్భుతమైన పని! మీరు అపరాధిని పట్టుకున్నారు.":"అయ్యో! మీరు అపరాధిని కనుక్కోలేకపోయారు! వచ్చే సారి కేసుతో బెటర్ లక్!";
		...
		//else{
				...
				var res= "అయ్యో! మీరు అపరాధిని కనుక్కోలేకపోయారు! వచ్చే సారి కేసుతో బెటర్ లక్!";
				isMatched = false;
				alertify.set({ labels: {ok: "కొనసాగించు"}  });//,onok:
				alertify.alert(''+ res,getFeedback1);
...
if(currentstage == 9 ){ jQuery( ".additionalTextVal").html("<br>	&ensp;	&ensp;	&ensp;	&ensp;	&ensp;మిషన్ ముగింపు!");}
