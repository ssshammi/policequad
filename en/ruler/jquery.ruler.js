			...
			if (settings.showCrosshair || settings.showMousePos) {
				...
				$('.grid').mousemove(function(e) {
					...
					//console.log(""+e.pageX-$(this).offset().left);
						/* $('.vMouse').css("top",e.pageY-$(this).offset().top);
						$('.hMouse').css("left",e.pageX-$(this).offset().left); */
							$('.vMouse').css("top",e.pageY-($(document).scrollTop())+1);
						$('.hMouse').css("left",e.pageX+1);
						//-($(window).scrollTop())
					}
					if (settings.showMousePos) {
						/* $(this).find('.mousePosBox').css({
						...
						var someX = e.pageX-$(this).offset().left;
						var someY =  e.pageY-$(this).offset().top;
						$('.mousePosBox').html(""+"x:" + someX + ", y:" +someY);
					}
				});
