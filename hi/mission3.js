...
function enableTools3(){
	...
	//rightAndge tool
	$j(CurrentMission+'.bucket:eq(0)').append('<select id="bucket1"><option value=""> </option><option value="atleast">कम से कम</option><option value="lessthan">"__" से कम</option><option value="morethan">"__" से अधिक</option><option value="exactly">केवल</option><option value="all">सब</option><option value="no">शून्य</option></select>');
	...
	*/
	$j(CurrentMission+'.bucket:eq(2)').append('<select id="bucket3"><option value="">             </option><option value="equal">समान</option><option value="parallel">समानांतर</option><option value="right">सम</option><option value="reflex">प्रतिवर्ती</option><option value="obtuse">अधिक</option><option value="acute">न्यून</option><option value="curved" disabled>वक्र </option></select>');
	...
	$j(CurrentMission+'.bucket:eq(3)').append('<select id="bucket4"><option value="">           </option><option value="sides">भुजाएँ</option><option value="angles">कोण</option><option value="pairsofsides">भुजाओं की जोड़ी</option>');
...
function EnableMission3(){
	...
	$j(CurrentMission+'.ask').on('click', function() { $j(".blockedEff").hide();$j(".allowedEff").hide(); 	$j('.ask').hide();
		...
		var QuestionText = 'जासूस भेजें जिसके पास ' +statment;
	...
	$j(CurrentMission+".tool2:eq(2)").qtip({    content: {text: 'अधिक कोण'}	});
	$j(CurrentMission+".tool2:eq(1)").qtip({    content: {text: 'समकोण'}	});
	$j(CurrentMission+".tool2:eq(0)").qtip({    content: {text: 'न्यून कोण'}	});
	$j(CurrentMission+".tool2:eq(3)").qtip({    content: {text: 'रिफ्लेक्स कोण'}	});
}
...
function callbackPuff3(){
	...
	if(currentScore <= coinsValue3[currentStage-1]){
			isMatched=false;
			shapeOrigin='Block';
			alertify.alert('आपने सारे टोकन्स खत्म कर दिए। अगले केस के लिए शुभकामनाएं!',showFeedback2);			
...
function GenerateShape(b1,b2,b3,b4){
	...
	{ 
		countNA++;
		alertify.error('यह वैध गुण नहीं है' );
...
$j( ".genrate" ).delay(1000).effect( "transfer",{ to: ".allowed", className: "ui-effects-transfer_allow" }, 1000).delay(800).effect( "transfer",{ to: ".allowed", className: "ui-effects-transfer_allow" }, 500);
	...
	$j(".lvlScreen .level2").html("लेवल "+ (currentLevelDisplay3[currentStage-1]+1));
 ...
 */	
	$j(".mtch_txt").html("अनुमति-प्राप्त"); $j(".job").html("बहुत बढिया!");$j(".solved").html("आप अपने जासूस अंदर भेजने में सफल रहे!");
...
//	}
	...
	$j( ".additionalTextVal").html("लगता है आपको एक और फैक्टरी में भेजने के लिए एक ज्युडिक जासूस खोजना होगा।");
	if(currentStage >= 5){
	jQuery( ".additionalTextVal").html("<br>	&ensp;	&ensp;	&ensp;	&ensp;	&ensp;मिशन अंत!");
	}
...
//$j( ".genrate" ).addClass("ui-effects-transfer_allow");
	...
	$j(".feed:eq(2) "+".scoretxt").effect( "highlight",200);
//setTimeout(function(){
	$j(".mtch_txt").html("ब्लॉक किया गया"); $j(".job").html("ओह नहीं!");$j(".solved").html("आपको कोई उचित ज्युडिक जासूस नहीं मिला!");
	 $j( ".additionalTextVal").show();
...
function startMission3(){ shapeOrigin='Blocked'; $j("#gloss_btn,#help_btn,#menu_btn").show(); startTimerlvl();
	...
	$j(CurrentMission+".mission").html("मिशन 3");
	$j(CurrentMission+".level").html("लेवल "+currentLevelDisplay3[currentStage-1]);
