  ...
  function selectedPoints(e) { 
    ...
    if (e.currentTarget.children[5].style.display === "block") {
      ...
      for(var i = 0; i < points.length; i++) {
		...
		alertify.alert("Select only the four vertices of the shape.");
	  }
    else {
      e.currentTarget.children[0].style.display = "none";
      e.currentTarget.children[1].style.display = "none";
...
function getFeedback4(){
	...
	if(MatchedH!="player"){ assessment_negative.play();$j(".mtch_txt").html("Enemy<br>Wins!"); $j(".job").html("Nice Try!");$j(".solved").html("Please attempt again!");
	$j( ".additionalTextVal").html("Try to solve the case again.");
	//$j( ".additionalTextVal").hide();
	}else{ assessment_positive.play();
	 $j(".mtch_txt").html("You<br>Win!"); $j(".job").html("Great work!");$j(".solved").html("You deactivated the enemy frame!");
	 $j( ".additionalTextVal").html("Looks like you have another case to solve.");
	 if(currentLevel == 5){
	  $j( ".additionalTextVal").html("<br>	&ensp;	&ensp;	&ensp;	&ensp;	&ensp;MISSION END!");
	 }
	...
	$j(".lvlScreen .level2").html("LEVEL "+ (currentLevelDisplay4[currentLevel]+1));
	//alert("ok");
 ...
 // alertify.alert(st +" wins!",getFeedback4);
  ...
  var plot = 'M '+vertices[0][0]*multiplier+' '+vertices[0][1]*multiplier+' L'+vertices[1][0]*multiplier+' '+vertices[1][1]*multiplier+' L'+vertices[2][0]*multiplier+' '+vertices[2][1]*multiplier+' L'+vertices[3][0]*multiplier+' '+vertices[3][1]*multiplier+' ';
	...
	if(st!="player")$j(".nextcase .txt").html("Play Again"); 
		//getFeedback4();
	alertify.alert((st=='player'?"You Win!":"Ra-Haka’s army Won!") ,getFeedback4);	
	//	MatchedH +" wins!"
