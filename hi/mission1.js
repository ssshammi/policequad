...
$("#feedback").hide();
$("#trans1").hide();
$j(".mission").html("मिशन  1");
//$j(".level").html("LEVEL "+currentstage);
$j(".level").html("लेवल "+currentLevelDisplay1[currentstage-1]);
...
$j(CurrentMission1+".ToolMain2:visible").freetrans('destroy');
	...
	$j(CurrentMission1+'.cluetext').html("क्लु पैनल");
		...
		alertify.confirm("Are you sure you want next clue?", function(e){ 
			if (e) {
				...
				// do nothing
					...
					alertify.error('टोकन खत्म, कोई आरोपी नहीं छोड़े गए।' );
					}
	...
	alertify.set({ labels: {ok: "ठीक", cancel : "नहीं"}  });
	if(cluesAsk.length ==0){
		alertify.alert('वहाँ कोई और अधिक सुराग नहीं  हैं!');
		return;
	} 
	var ExistShapes =[];
	...
	if(actualAllowedClueCount==cluesAsk.length){	
		alertify.alert('सभी क्लु खत्म हो गए हैं');		
		return;
	}
	...
	var QuestionText = 'अपराधी के पास ' + appendStatement;
...
function enableReleaseCLICK(){
 $j(CurrentMission1+'.release').click(function() { 	 $j(CurrentMission1 +'.ask').show();  
			...
			if( $j(CurrentMission1+'.grid.selected').length >= TotalShapes){alertify.alert('सभी को रिहा नहीं कर सकते. इनमें से एक आपराधि हैं!');
			 return;
			 }
			if( $j(CurrentMission1+'.grid.selected').not('.released').length ==0){alertify.alert('कोई आरोपी नहीं छोड़े गए।');
			 return;
			 }
				...
				if(!$j(CurrentMission1+'.grid.selected.released').length == TotalShapes-1){
					$j(CurrentMission1+'.grid.selected.released').each(function(){
						if($j(this).prop('fig') ==currentCulpret1){
							alertify.alert("ओहो .. लगता है कि आपने अपराधी को भी रिहा कर दिया! अगले केस के लिए बेहतर प्रयास कीजिये।",getFeedback1);
							return;
						}
					});
				}
	 ...
	 //$j(CurrentMission1+'.release').trigger("click");
		...
		alertify.set({ labels: {ok: "हाँ", cancel : "नहीं"}  });
		alertify.confirm("आप का केस अभी सुल्ज़ा नहीं है.. क्या आप यह केस समाप्त करना चाहते है? ", function(e){ 
			if (e) {
				//resetLevel1();
				var res= "ओहो .. आप अपराधी को ढूंढ़ नही पाए। अगले केस के लिए बेहतर प्रयास कीजिये।";
				isMatched = false;
				alertify.set({ labels: {ok: "आगे बढ़ें ",cancel : "हाँ"}  });//,onok:
		...
		if($j(CurrentMission1+'.grid.selected.released').length == TotalShapes-1){
				...
				var res= (culpret== currentCulpret1 )?"बहुत अच्छे! आपने अपराधी को पकड़ लिया":"ओहो .. आप अपराधी को ढूंढ़ नही पाए। अगले केस के लिए बेहतर प्रयास कीजिये।";
				isMatched = (culpret== currentCulpret1 );
				alertify.set({ labels: {ok: "आगे बढ़ें ",cancel : "नहीं"}  });//,onok:
				alertify.alert(''+ res,getFeedback1);
				 return;
				}
		//else{
				...
				var res= "ओहो .. आप अपराधी को ढूंढ़ नही पाए। अगले केस के लिए बेहतर प्रयास कीजिये।";
				isMatched = false;
				alertify.set({ labels: {ok: "आगे बढ़ें ",cancel : "नहीं"}  });//,onok:"वापसी"
				alertify.alert(''+ res,getFeedback1);
...
function getFeedback1(){
	...
	if(!isMatched){ $j(".mtch_txt").html("कोई मेल नहीं"); $j(".job").html("कोई बात नहीं! ");$j(".solved").html("फिर कोशिश करें!");
	$j( ".additionalTextVal").html("नया केस सुलझाएं");
		//$j( ".cont_txt").html("CONTINUE");
	incorrectRelease.play();
	}else{  	assessment_positive.play();
	 $j(".mtch_txt").html("मेल"); $j(".job").html("शाबाश!");$j(".solved").html("आपने केस सुलझा लिया");
	 /* if(currentstage != 3)
...
if(currentstage == 9 ){ jQuery( ".additionalTextVal").html("<br>	&ensp;	&ensp;	&ensp;	&ensp;	&ensp;मिशन अंत!");}
	...
	$j(".lvlScreen .level2").html("लेवल "+ (currentLevelDisplay1[currentstage-1]+1));
...
function callbackPuff1(){
	...
	if(currentScore <= coinsValue1[currentstage-1]){
		isMatched =false;
		alertify.alert('आपने सारे टोकन्स खत्म कर दिए। अगले केस के लिए शुभकामनाएं! ',getFeedback1);
